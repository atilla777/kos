...
The local `kos-repository` adapter accepts only its closed version 1 JSON documents for `materialize`, `observe`, and `remove`. Each request carries the registered repository identity and canonical common directory, the current reservation identity and fencing token, and an explicit expected SHA-1 commit. Materialization requires a `reserved` snapshot and verifies that the registered base ref still names the expected commit before creating the task branch. Removal requires `release_pending`. The runtime-facing skill that supplies these documents is a separate integration boundary.
...
Repository mutations are serialized by a lock in the registered common directory. Git runs with bounded time, argument arrays, prompts and pagers disabled, global and system configuration excluded, hooks disabled, replacement objects disabled, and lazy object fetching disabled. Version 1 rejects repositories whose object format is not SHA-1. Cleanup unlocks an adapter-owned worktree only immediately before ordinary non-forced `git worktree remove`; it never recursively deletes the path or deletes the task branch.
