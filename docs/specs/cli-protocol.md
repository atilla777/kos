...
kos repository get --repository <repository-id> --json
kos runtime-config get --json
kos runtime-config update --input <path|-> --idempotency-key <key> --json
```
...
| `repository.get` | `kos repository get` | `GET /api/v1/repositories/{repository_id}` | `200` repository |
...
`runtime_config.get` returns the one closed global resource containing `retrospective_enabled`, its optimistic `lock_version`, and update time. Retrospective is disabled by default. `runtime_config.update` accepts the requested boolean and `expected_lock_version`; it uses global idempotency and rejects concurrent stale updates with `stale_lock_version`. `kos-opencode` samples this resource once before starting a new orchestration. The setting is not reread during that orchestration and does not rewrite a workflow-step context already frozen by `step.context`.
...
`repository.get` returns that complete immutable registered repository resource in its repository scope. `worktree.get` returns the reservation allocation and owner plus lifecycle-appropriate persisted common-directory digest, HEAD, latest observation state and digest, confirmation and release times, and update time. These reads provide authoritative snapshots for adapter requests; they do not inspect Git or refresh observations. Newly serialized resources contain these applicable fields; the schema keeps them optional so a completed pre-upgrade idempotency replay remains valid with its original response data.
...
Task creation accepts only a version 1 `task_input` and the `quick-fix` task type. The closed task input contains a nonblank title and a nonblank approved Markdown brief of at most 128 KiB. In its transaction, the server stores the immutable input, resolves the task type's current published version, and stores its immutable `workflow_version_id` and initial state on the task. A client cannot select or assert a workflow version. Later activation changes do not affect that task.

A task-input title and approved brief must each contain at least one non-whitespace character. Creation returns `task_type_unavailable` without allocating a number when the task type has no active published workflow version. After sequence `999999`, creation returns `task_number_exhausted` without widening, wrapping, or changing the exhausted sequence. Both failures are nonretryable conflicts.

After claim and confirmation of any required worktree, `step.context` constructs the complete executable context from the task, active attempt, current status, and pinned workflow version. Before review context capture, the orchestrator must freshly observe and reconcile the exact current candidate under the review attempt's fencing token. Successful initial review context capture adds a server-generated UUID nonce to the immutable context and atomically clears the reservation's mutable latest observation state and digest after validating them, while preserving its confirmed identity and HEAD; a later idempotent context replay returns the frozen nonce and context without consuming a newer observation. The nonce makes the resulting input-context digest unavailable before context finalization. A publication attempt first prepares or adopts its durable publication resource. Until that resource and operation are implemented, `step.context` returns `context_unavailable` for publication rather than returning a partial context. In one transaction `step.context` stores the immutable context and its digest on the attempt and returns the exact persisted task input, exact UTF-8 Markdown instruction, optional inline artifact templates, artifact requirements, allowed typed repository effects, installation retrospective setting, and prepared publication parameters when applicable. A legacy task without task input returns `context_unavailable` before storing context. Each required artifact contains its type, cardinality (`one` or `many`), subject (`task` or `candidate`), and nonempty allowed-state set. Requirements are unique by type. An instruction and an approved brief are each at most 128 KiB, each template is at most 1 MiB, and one state's instruction and templates total at most 4 MiB, measured over UTF-8 bytes.
...
`effect.prepare` stores the exact request and canonical digest before commit, fetch, or rebase. Only then may the orchestrator invoke `kos-repository`. `effect.reconcile` records a typed success or failure; an unavailable or lost adapter response records an `unknown` state and requires observation of the expected HEAD, refs, index, and worktree before retry or terminal reconciliation. After a successful commit is reconciled, the orchestrator observes a clean worktree at that exact commit SHA and records the observation through `worktree.reconcile` before returning the effect result to the executor. This durable HEAD synchronization allows the next status to freeze current worktree state; it does not authorize a second worktree-bound effect from the old frozen context. A task may retain multiple generic effects. `prepared` and `unknown` effects are unresolved; `succeeded` and `failed` effects are terminal and immutable. An owning attempt cannot fail, enter needs-human, or complete its step while one of its generic effects remains unresolved.
...
After lease expiry, `task.get` exposes `active_publication_id`. The new orchestrator first reconciles the expired attempt, then claims the same task and workflow status. That claim atomically adopts each unresolved durable intent by setting its `current_owner_attempt_id` to the new attempt without changing its prepared parameters. Without a durable publication result, the new owner uses `publication.get`, observes the remote through `kos-repository`, and reconciles the existing publication. With a durable result, it instead retrieves that immutable result through the additive version 2 read and resumes only worktree cleanup without another observation or push. A second `publication.prepare` is rejected while an unresolved publication exists.
