...
A `WorkflowAttempt` represents one execution of the current workflow status. It records an attempt ID, task and status, owner ID, idempotency key, monotonic fencing token, state (`started`, `succeeded`, `failed`, `interrupted`, or `needs_human`), start, heartbeat, and completion times, the immutable executable context and input-context digest after context finalization, the result manifest after submission, and immutable reconciliation observation after recovery of an expired attempt.

Before mutating repository files or attempt-owned KOS state, an orchestrator atomically claims a lease for the task, workflow status, and expected lock version through the CLI. The lease has a bounded lifetime, can be renewed by the holder of its attempt ID and fencing token, and is released when the attempt completes, enters `needs_human`, or expires. Renewal starts a fresh bounded lease at its recorded heartbeat; an expiry equal to the current time is already expired. After expiry, a new orchestrator reconciles the unfinished attempt before creating another. Claim rejects both live and expired unreconciled attempts. Every mutation owned by an active attempt and every `kos-repository` operation carries the current fencing token; task creation, attempt claim, and expired-attempt reconciliation do not. Fencing mismatch is checked before lease expiry, and an attempt with a stale token cannot complete the step. An attempt also cannot fail, enter `needs_human`, or complete while it owns a prepared or unknown generic repository effect. Exact command preconditions are defined by [CLI Protocol Version 1](cli-protocol.md).

Attempt failure does not change workflow status. Failed and needs-human artifacts remain in the immutable result manifest but are not registered as transition artifacts. KOS records durable intent before an external operation and, after interruption, reconciles observed state instead of blindly repeating the operation. Attempt reconciliation stores the observation state, evidence digest, and time as immutable recovery evidence, marks the expired attempt interrupted, and does not assert that an external effect succeeded. [ADR-0002](../decisions/0002-recoverable-workflow-attempts.md) records this recovery decision.
...
After claim and confirmation of any required worktree, the orchestrator finalizes an attempt-bound versioned context envelope with the idempotent `kos step context` mutation. For a publication status, it first prepares or adopts the durable publication intent so the context can include its identifier, candidate, remote, base ref, and expected remote OID. KOS stores the immutable input before returning it. The context contains the exact persisted versioned approved task input; task and attempt identities; workflow-version and state identities; the exact pinned Markdown instruction and optional artifact templates; expected lock version; fencing token; repository identity; worktree; base ref; candidate SHA; optional prepared-publication context; required artifacts; allowed repository effects; and the installation retrospective setting. A legacy task without approved input cannot finalize a new context. A new step kind is introduced by a published instruction, not a new runtime skill.

The generic executor may synchronously request one allowed typed repository effect through its owning orchestrator before finalizing its result. The orchestrator verifies the active lease, fencing token, input-context digest, effect allowlist, and operation preconditions. Commit, fetch, and rebase requests are persisted as generic repository-effect intents before `kos-repository` runs; worktree removal and push use their existing reservation and publication intents. The orchestrator reconciles the typed success, failure, or unknown observation and returns it to the same executor session. For OpenCode, the runtime adapter retains the foreground Task child identifier from the runtime-generated completion wrapper and resumes that exact session with the effect result; the identifier is not accepted from the nested child JSON or persisted as KOS workflow state. This round trip lets the executor bind document, candidate, test, and publication evidence to actual Git state. An effect failure remains explicit input to the executor; it is not silently converted into success.
...
When sampled as enabled at the start of a KOS OpenCode orchestration, retrospective is best-effort runtime post-processing after every gracefully ending orchestration or workflow-step subagent session. The plugin derives child eligibility only from the exact `kos-workflow-step` Task route. After durably handling the primary manifest, the orchestrator invokes `child_retrospective` with only the retained `child_session_id`; this post-handling call is the acknowledgement signal rather than model-supplied booleans. `kos-opencode` invokes the root with an external second prompt only after the primary process completes. It is not a workflow state or attempt, does not modify the source task, and cannot replace or downgrade a primary result. At most five sanitized child results are retained in current orchestration memory. Runtime structural leakage checks backstop, but cannot prove, model sanitization. A missing retrospective transport result is distinct from a valid `no_action` result. [KOS Retrospective](retrospective.md) defines eligibility, ordering, privacy, proposal classification, and failure behavior.
