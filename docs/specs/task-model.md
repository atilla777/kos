...
Every task receives an immutable public number such as `TASK-000123`. The SQLite primary key is internal. Public numbers are allocated within the task-creation write transaction, protected by a uniqueness constraint, never reused within a project, and used by the CLI, branch names, task-artifact paths, Git trailers, and human or agent references. Child tasks have their own numbers; relations, not composite numbers, define hierarchy.
