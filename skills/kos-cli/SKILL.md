...
| --- | --- | --- |
| Global | `task-type list`, `workflow list`, `workflow get`, `workflow export`, `workflow-draft get`, `workflow-draft validate` | `workflow-draft import`, `workflow publish`, `workflow activate` |
...
`repository register`, `runtime-config get`, `runtime-config update`, `artifact register`, and `publication complete` exist in the wider protocol design but are not available in the current CLI. Do not invoke them or simulate them through direct API access.
