...
2. Before a publication context, read or prepare the one durable publication for the exact reviewed candidate, registered trusted remote, full base ref, and expected remote OID. Use `publication prepare` only when every value is available from authoritative KOS state and approved evidence. Never infer the remote tip or bootstrap it through an unauthorized fetch.
3. Invoke `step context` only after every required worktree is confirmed and every required publication is prepared or adopted. Retain the exact command body and idempotency key.
4. Accept only a complete document validated as `schemas/cli/v1/workflow.json#/$defs/context`. Require `schema_version: "1"` and exact task, repository, workflow version, workflow status, attempt, lock version, fencing token, base ref, candidate, worktree, and publication agreement with the pinned definition and current state wherever each field applies.
5. Recompute `input_context_digest` as SHA-256 over the RFC 8785 canonical JSON serialization of the complete context with only `input_context_digest` omitted. Stop before execution on any mismatch, missing field, partial context, or unexpected field.
...
| `rebase` | Verify the exact reservation and frozen HEAD and bind `onto_sha` to complete successful evidence from the authorized fetch; call `effect prepare`, adapter `rebase`, then `effect reconcile`. |
...
- A publication artifact requires the specialized `publication complete` protocol after successful remote evidence and released worktree. That command is unavailable in the current CLI, so stop with the reconciled publication and preserved manifest; never pass publication evidence to `step complete` or simulate completion.
