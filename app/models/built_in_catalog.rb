class BuiltInCatalog
  ...
  CATALOG = {
    "brief" => {
      ...
      steps: [
        [ "brief", "Brief", "advanced", "Specify the requested product behavior and graph, then leave a clean local task commit sequence without pushing.",
        ...
        [ "publish", "Publish", "standard", "Push the exact reviewed specification commits unchanged, observe them remotely, and materialize the validated child graph.",
    ...
    "development" => {
      ...
      steps: [
        ...
        [ "implement", "Implement", "standard", "Implement the plan, run every required check, and leave clean local task commits without pushing.",
        ...
        [ "review", "Review", "advanced", "Independently review and record the exact clean commit sequence without modifying the repository.",
          "# Review\n\n## Commit sequence\n\n## Trees and diff digest\n\n## Findings\n\n## Decision", { "approved" => { "next_step" => "publish" },
            "changes_requested" => { "next_step" => "implement" }, "redesign_required" => { "next_step" => "plan" } } ],
        [ "publish", "Publish", "standard", "Validate and push the exact reviewed commit sequence unchanged, then observe it remotely.",
    ...
    "fix" => {
      ...
      steps: [
        ...
        [ "implement", "Implement", "standard", "Implement the fix and regression check, run every required check, and leave clean local task commits without pushing.",
          "# Implementation\n\n## Changes\n\n## Checks\n\n## Commit sequence", { "implemented" => { "next_step" => "document" },
            "plan_invalid" => { "next_step" => "plan" } } ],
        [ "document", "Document", "standard", "Update affected product specifications through OKF, or record why observable behavior did not change.",
          "# Documentation\n\n## Product behavior", { "documented" => { "next_step" => "review" },
            "implementation_invalid" => { "next_step" => "implement" } } ],
        [ "review", "Review", "advanced", "Independently review the diagnosis and exact clean commit sequence without modifying the repository.",
          "# Review\n\n## Commit sequence\n\n## Trees and diff digest\n\n## Findings\n\n## Decision", { "approved" => { "next_step" => "publish" },
            "changes_requested" => { "next_step" => "implement" }, "redesign_required" => { "next_step" => "plan" } } ],
        [ "publish", "Publish", "standard", "Validate and push the exact reviewed fix commit sequence unchanged, then observe it remotely.",
          "# Publication\n\n## Reviewed commit sequence\n\n## Remote observation", { "published" => { "complete_task" => true },
            "review_invalid" => { "next_step" => "review" }, "base_moved" => { "next_step" => "implement" } } ]
      ]
    }
