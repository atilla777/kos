...
The local repository adapter reads a closed version 1 request from a file or stdin and emits exactly one JSON result. Worktree operations carry the registered repository and reservation snapshots; fetch and rebase also carry registered trust, durable effect request, and current ownership snapshots:
...
bin/kos-repository rebase --input rebase.json --json
bin/kos-repository push --input push.json --json
```
...
`rebase` requires a matching confirmed clean worktree and binds `onto_sha` to the canonical request and result evidence from a verified fetch of the registered base ref. It replays only a linear suffix of commits carrying the reservation-derived `KOS-Task` trailer and only when the fetched base descends from the task's original base. Rebase uses fixed non-interactive semantics and rejects executable filters, custom merge drivers, and configured rebase strategies. An ordinary conflict is aborted and returned as terminal only after exact clean restoration is verified; an unprovable result or restoration is retryable and uncertain. Success returns the resulting HEAD and canonical evidence after checking ancestry, task trailers, refs, index, worktree, and unfinished-operation state. Durable reservation-HEAD synchronization and runtime orchestration remain separate boundaries.

`push` carries the registered trust and prepared publication ownership snapshots. It fetches the trusted base ref without writing shared `FETCH_HEAD`, returns without pushing when the candidate is already reachable or the base moved, and otherwise combines a verified fast-forward relation with an exact expected-old-OID lease while updating only the configured base ref. Every attempted push is followed by another bounded trusted fetch. A valid observation returns the remote tip, candidate reachability, timestamp, and canonical evidence even when push was rejected; inability to observe after push begins returns retryable `unknown` and prohibits blind retry. Push disables signing and Trace2, rejects configuration includes and redirecting or executable repository configuration, changes no local refs, index, or worktree, and never emits raw Git diagnostics or remote credentials.
