...
RSpec.describe WorkflowSteps::Complete, :aggregate_failures do
  ...
  def prepare_state(environment)
    ...
    JSON.parse(run_script(environment, <<~RUBY))
      ...
      task = Task.create!(repository: repository, sequence: 1, title: "Concurrent completion",
        task_input_schema_version: "1", approved_brief: "Complete the approved task concurrently.", task_type: type,
