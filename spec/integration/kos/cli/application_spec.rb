...
RSpec.describe Kos::Cli::Application, :aggregate_failures do
  ...
  it "maps runtime configuration reads to the global endpoint" do
    expect_runtime_config_read
  end

  def expect_runtime_config_read
    config = { "schema_version" => "1", "retrospective_enabled" => false, "lock_version" => 0,
      "updated_at" => "2026-09-14T12:00:00Z" }
    result = success("runtime_config.get", config)
    with_server([ [ "200 OK", result ] ]) do |url, requests|
      stdout, _stderr, status = run_cli(url, "runtime-config", "get", "--json")

      expect([ status.exitstatus, JSON.parse(stdout), requests.first.first ])
        .to eq([ 0, result, "GET /api/v1/runtime-config HTTP/1.1\r\n" ])
    end
  end
  ...
  def mutation_cases
    ...
    [
      ...
      [ %w[runtime-config update], "runtime_config.update", "/api/v1/runtime-config",
        { "retrospective_enabled" => true, "expected_lock_version" => 0 } ],
      ...
      [ [ "step", "context", "--repository", repository_id ], "step.context",
        "/api/v1/repositories/#{repository_id}/attempts/#{resource_id}/step-context",
        { "preconditions" => preconditions } ],
      [ [ "worktree", "reserve", "--repository", repository_id ], "worktree.reserve",
        "/api/v1/repositories/#{repository_id}/tasks/KOS-000001/worktree-reservations",
        { "task_number" => "KOS-000001", "branch" => "kos/task-KOS-000001",
          "path" => "/tmp/worktrees/KOS-000001", "preconditions" => preconditions } ],
      [ [ "worktree", "confirm", "--repository", repository_id ], "worktree.confirm",
        "/api/v1/repositories/#{repository_id}/worktree-reservations/#{resource_id}/confirm",
        { "reservation_id" => resource_id, "git_common_dir_digest" => "sha256:#{'a' * 64}",
          "head_sha" => "b" * 40, "preconditions" => preconditions } ],
      [ [ "worktree", "reconcile", "--repository", repository_id ], "worktree.reconcile",
        "/api/v1/repositories/#{repository_id}/worktree-reservations/#{resource_id}/reconcile",
        { "reservation_id" => resource_id, "observed_state" => "dirty",
          "evidence_digest" => "sha256:#{'a' * 64}", "preconditions" => preconditions } ],
      [ [ "worktree", "release", "--repository", repository_id ], "worktree.release",
        "/api/v1/repositories/#{repository_id}/worktree-reservations/#{resource_id}/release",
        { "reservation_id" => resource_id, "observed_state" => "absent",
          "evidence_digest" => "sha256:#{'a' * 64}", "preconditions" => preconditions } ],
      [ [ "effect", "prepare", "--repository", repository_id ], "effect.prepare",
        "/api/v1/repositories/#{repository_id}/tasks/KOS-000001/repository-effects",
        { "task_number" => "KOS-000001", "effect_request" => {
          "schema_version" => "1", "attempt_id" => resource_id,
          "input_context_digest" => "sha256:#{'a' * 64}",
          "effect" => { "operation" => "fetch", "remote" => "origin", "ref" => "refs/heads/main" }
        }, "preconditions" => preconditions } ],
      [ [ "effect", "reconcile", "--repository", repository_id ], "effect.reconcile",
        "/api/v1/repositories/#{repository_id}/repository-effects/#{resource_id}/reconcile",
        { "effect_id" => resource_id, "effect_result" => {
          "schema_version" => "1", "effect_intent_id" => resource_id,
          "request_attempt_id" => resource_id, "owner_attempt_id" => resource_id,
          "input_context_digest" => "sha256:#{'a' * 64}",
          "effect_request_digest" => "sha256:#{'b' * 64}",
